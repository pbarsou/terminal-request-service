...
public class LambdaTerminalReservationServiceAdapter implements TerminalReservationServicePort {
  ...
  public LambdaTerminalReservationServiceAdapter(
  ...
  public UUID reserveATerminal(TerminalType terminalType, UUID terminalRequestId) {
    try {
      ...
      return objectMapper
          .readValue(response.body(), LambdaTerminalReservationResponse.class)
          .terminalId();

    } catch (Exception exception) {
      throw new TerminalReservationFailureException(
          "Failed to process terminal reservation", exception);
    }
  }
