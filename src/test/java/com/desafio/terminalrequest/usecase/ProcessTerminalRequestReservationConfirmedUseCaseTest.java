...
class ProcessTerminalRequestReservationConfirmedUseCaseTest {
    ...
    @Mock
    private DeliverySchedulingServicePort deliveryService;
    ...
    @BeforeEach
    void setUp() {
        useCase = new ProcessTerminalRequestReservationConfirmedUseCase(terminalRequestService, deliveryService, publisher);
    }
    ...
    void shouldScheduleDeliveryAndUpdateStatusToAgendadoWhenReservationIsConfirmed() {
        ...
        when(deliveryService.scheduleDelivery(event.address(), event.terminalRequestId(), event.terminalId()))
                .thenReturn(trackingId);

        useCase.execute(event);

        verify(deliveryService).scheduleDelivery(event.address(), event.terminalRequestId(), event.terminalId());
        verify(terminalRequestService).updateStatus(event.terminalRequestId(), TerminalRequestsStatus.AGENDADO);
    ...
    void shouldUpdateStatusToErroAgendamentoAndPublishFailedEventWhenDeliverySchedulingFails() {
        ...
        verify(terminalRequestService).updateStatus(event.terminalRequestId(), TerminalRequestsStatus.ERRO_AGENDAMENTO);
        verify(terminalRequestService, never()).assignTracking(any(), any());

        ArgumentCaptor<TerminalRequestDeliverySchedulingFailed> eventCaptor = ArgumentCaptor.forClass(
                TerminalRequestDeliverySchedulingFailed.class
        );
